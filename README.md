...
- **社群发现**：Louvain 算法（局部移动 + 社区聚合），前端按社群着色；支持限定最终社群个数 / 合并过小社群（零散小社群自动并入连接最强的邻近社群），并可仅在界面上隐藏过小社群（不改底层数据）；不传参数时划分结果与原始 Louvain 完全一致
...
| GET  | `/api/communities` | Louvain 社群划分（可选 `max_communities`、`min_size` 参数合并小社群，不传则结果与原始划分一致） |
...
- **小社群合并（可选后处理）**：`max_communities` 限定最终社群个数、`min_size` 合并规模过小的社群；每轮将最小社群并入与其边权总和最大的邻近社群（孤立社群并入最大社群），平局按编号裁决，结果确定可复现；两参数均缺省时原样返回，不改变原始划分
